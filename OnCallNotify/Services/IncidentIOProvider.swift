...
class IncidentIOProvider: ServiceProvider {
    private(set) var account: Account
    private let baseURL = "https://api.incident.io/v2"
    ...
    func acknowledgeIncident(incidentId: String) async throws {
        ...
        // Incident.io uses incident status updates to change status to "investigating"
        // This is the equivalent of acknowledging an incident
        let endpoint = "/incidents/\(incidentId)/actions/update_status"
        let url = try buildURL(endpoint: endpoint)
        var request = try buildRequest(url: url, apiToken: apiToken)
        request.httpMethod = "POST"

        // Set status to "investigating" (acknowledged equivalent in Incident.io)
        let body: [String: Any] = [
            "new_status_id": "investigating"  // May need to fetch actual status IDs
        ]
    ...
    private func fetchCurrentUser(apiToken: String) async throws {
        ...
        // Parse user list and get first user ID (representing current user)
    ...
    private func fetchIncidents(apiToken: String) async throws -> [Incident] {
        ...
        return try parseIncidentIOIncidents(from: data)
    }

    private func checkOnCallStatus(apiToken: String) async throws -> Bool {
        // TODO: Implement Incident.io on-call schedule checking
        // Incident.io on-call is managed through schedules API
        // Would need to fetch schedules and check if current user is on call
    ...
    private func parseIncidentIOIncidents(from data: Data) throws -> [Incident] {
        guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let incidentsData = json["incidents"] as? [[String: Any]] else {
            return []
        }
        ...
        for incidentData in incidentsData {
            guard let id = incidentData["id"] as? String,
                  let name = incidentData["name"] as? String,
                  let createdAt = incidentData["created_at"] as? String else {
                continue
            }

            // Map Incident.io status to our status
            let statusString = incidentData["status"] as? String ?? "triage"
            let status: IncidentStatus
            switch statusString {
            case "triage":
                status = .triggered
            case "investigating", "fixing":
                status = .acknowledged
            ...
            let severity = incidentData["severity"] as? [String: Any]
            let urgency = (severity?["name"] as? String)?.lowercased() ?? "high"
            ...
            let incident = Incident(
                ...
                htmlUrl: incidentData["permalink"] as? String,
                incidentNumber: nil,
                service: nil,
                assignments: nil,
