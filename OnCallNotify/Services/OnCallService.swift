...
class OnCallService: ObservableObject {
    ...
    private var accountServices: [String: OnCallServiceProvider] = [:] // accountId -> service
    ...
    func initializeAccountServices() {
        ...
        for account in accounts where account.isEnabled {
            if accountServices[account.id] == nil {
                accountServices[account.id] = createService(for: account)
            } else {
    ...
    private func createService(for account: Account) -> OnCallServiceProvider {
        switch account.serviceType {
        case .pagerDuty:
            return PagerDutyService(account: account)
        case .alertOps:
            return AlertOpsService(account: account)
        }
    }
